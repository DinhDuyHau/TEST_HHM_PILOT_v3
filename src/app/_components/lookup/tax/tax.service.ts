...
export class TaxService implements IGridService<Tax>{
    filter !: ItemFilter[];
    name?: string;
    ...
    getTitle(): string {
        return 'Danh mục thuế';
    }
    getItems(page: { pageIndex: number; pageSize: number; }, filter: ItemFilter[], sort: ItemSort): Observable<Result<Tax>> {
        if (filter) {
            if (this.filter) {
        ...
        return this.http.post<Result<Tax>>(environment.apiUrl +
            `/Category/find/dmthue?order_by=${order_by}&page_index=${page.pageIndex + 1}&page_size=${page.pageSize}`, body
        ).pipe();
    }
    getItem(id: string): Observable<Tax> {
        return this.getItems({ pageIndex: 0, pageSize: 1 }, [id !== '' ? { name: 'ma_thue', value: id } : { name: '1', value: '1' }], { name: 'ma_thue', direction: 'asc' }).pipe(switchMap(item => {
    ...
    getFields(): Observable<Field[]> {
        ...
        return this.http.get<Field[]>(`assets/fields/lookup/dmthue.json?r=${randomParam}`).pipe(
            switchMap(data => {
                data = data.map((item) => {
