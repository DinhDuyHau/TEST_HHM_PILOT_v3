...
export class LevelService implements IGridService<Level>{
  filter !: ItemFilter[];
  name?: string;
  ...
  getTitle(): string {
    return 'Danh mục mức độ';
  }
  getItems(page: { pageIndex: number; pageSize: number; }, filter: ItemFilter[], sort: ItemSort): Observable<Result<Level>> {
    if (filter) {
      if (this.filter) {
    ...
    return this.http.post<Result<Level>>(environment.apiUrl +
      `/Category/find/dmmd?order_by=${order_by}&page_index=${page.pageIndex + 1}&page_size=${page.pageSize}`, body
    ).pipe();
  }
  getItem(id: string): Observable<Level> {
    // return this.http.post<Level>(environment.apiUrl +
    //   '/Category/getbyid/dmmd', { name: 'ma_md', operator: '=', value: id }
    // ).pipe();

    return this.getItems({ pageIndex: 0, pageSize: 1 }, [id !== '' ? { name: 'ma_md', value: id } : { name: '1', value: '1' }], { name: 'ma_md', direction: 'asc' }).pipe(switchMap(item => {
  ...
  getFields(): Observable<Field[]> {
    ...
    return this.http.get<Field[]>(`assets/fields/lookup/dmmd.json?r=${randomParam}`).pipe(
      switchMap(data => {
        data = data.map((item) => {
