...
export class ExchangeRateService implements IGridService<ExchangeRate>{
  filter !: ItemFilter[];
  name?: string;
  ...
  getItems(page: { pageIndex: number; pageSize: number; }, filter: ItemFilter[], sort: ItemSort): Observable<Result<ExchangeRate>> {
    if (filter) {
      if (this.filter) {
    ...
    return this.http.post<Result<ExchangeRate>>(environment.apiUrl +
      `/Category/find/dmnt?order_by=${order_by}&page_index=${page.pageIndex + 1}&page_size=${page.pageSize}`, body
    ).pipe();
  }
  getItem(id: string): Observable<ExchangeRate> {
    return this.getItems({ pageIndex: 0, pageSize: 1 }, [id !== '' ? { name: 'ma_nt', value: id } : { name: '1', value: '1' }], { name: 'ma_nt', direction: 'asc' }).pipe(switchMap(item => {
  ...
  getFields(): Observable<Field[]> {
    ...
    return this.http.get<Field[]>(`assets/fields/lookup/dmnt.json?r=${randomParam}`).pipe(
      switchMap(data => {
        data = data.map((item) => {
