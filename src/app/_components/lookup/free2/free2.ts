...
export class Free2Service implements IGridService<CommisstionCollection>{
  filter!: ItemFilter[];
  name?: string;
  ...
  getItems(page: { pageIndex: number; pageSize: number; }, filter: ItemFilter[], sort: ItemSort): Observable<Result<CommisstionCollection>> {
    ...
    return this.http.post<Result<CommisstionCollection>>(environment.apiUrl +
      `/Category/find/dmtd2?order_by=${order_by}&page_index=${page.pageIndex + 1}&page_size=${page.pageSize}`, body
    ).pipe();
  }
  getItem(id: string): Observable<CommisstionCollection> {
    // return this.http.post<CommisstionCollection>(environment.apiUrl +
    //   '/Category/getById/dmtd2', { name: 'ma_td', operator: '=', value: id }
    // ).pipe();

    return this.getItems({ pageIndex: 0, pageSize: 1 }, [id !== '' ? { name: 'ma_td', value: id } : { name: '1', value: '1' }], { name: 'ma_td', direction: 'asc' }).pipe(switchMap(item => {
  ...
  getFields(): Observable<Field[]> {
    ...
    return this.http.get<Field[]>(`assets/fields/lookup/dmtd2.json?r=${randomParam}`).pipe(
      switchMap(data => {
        data = data.map((item) => {
