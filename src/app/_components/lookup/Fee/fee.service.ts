...
export class FeeService implements IGridService<Fee>{
  filter!: ItemFilter[];
  name?: string;
  ...
  getTitle(): string {
    return 'Danh mục chi phí';
  }
  getItems(page: { pageIndex: number; pageSize: number; }, filter: ItemFilter[], sort: ItemSort): Observable<Result<Fee>> {
    if (filter) {
      if (this.filter) {
    ...
    return this.http.post<Result<Fee>>(environment.apiUrl +
      `/Category/find/dmphi?order_by=${order_by}&page_index=${page.pageIndex + 1}&page_size=${page.pageSize}`, body
    ).pipe();
  }
  getItem(id: string): Observable<Fee> {
    // return this.http.post<Fee>(environment.apiUrl +
    //   '/Category/getById/dmphi', { name: 'ma_tt', operator: '=', value: id }
    // ).pipe();
  ...
  getFields(): Observable<Field[]> {
    ...
    return this.http.get<Field[]>(`assets/fields/lookup/dmphi.json?r=${randomParam}`).pipe(
      switchMap(data => {
        data = data.map((item) => {
