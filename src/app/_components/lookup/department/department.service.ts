...
export class DepartmentService implements IGridService<Department>{
  ...
  getTitle(): string {
    return 'Danh mục bộ phận';
  }
  getItems(page: { pageIndex: number; pageSize: number; }, filter: ItemFilter[], sort: ItemSort): Observable<Result<Department>> {
    let order_by = '';
    if (sort && sort.name !== '') {
      if (sort.direction !== '') {
    ...
    return this.http.post<Result<Department>>(environment.apiUrl +
      `/Category/find/dmbp?order_by=${order_by}&page_index=${page.pageIndex + 1}&page_size=${page.pageSize}`, body
    ).pipe();
  }
  getItem(id: string): Observable<Department> {
    return this.getItems({ pageIndex: 0, pageSize: 1 }, [id !== '' ? { name: 'ma_bp', value: id } : { name: '1', value: '1' }], { name: 'ma_bp', direction: 'asc' }).pipe(switchMap(item => {
  ...
  getFields(): Observable<Field[]> {
    ...
    return this.http.get<Field[]>(`assets/fields/lookup/dmbp.json?r=${randomParam}`).pipe(
      switchMap(data => {
        data = data.map((item) => {
