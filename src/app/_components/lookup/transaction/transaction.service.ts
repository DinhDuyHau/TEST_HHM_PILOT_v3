...
export class TransactionService implements IGridService<Transaction>{
  filter !: ItemFilter[];
  name?: string;
  ...
  getItems(page: { pageIndex: number; pageSize: number; }, filter: ItemFilter[], sort: ItemSort): Observable<Result<Transaction>> {
    if (filter) {
      if (this.filter) {
    ...
    return this.http.post<Result<Transaction>>(environment.apiUrl +
      `/Category/find/dmmagd?order_by=${order_by}&page_index=${page.pageIndex + 1}&page_size=${page.pageSize}`, body
    ).pipe();
  }
  getItem(id: string): Observable<Transaction> {
    return this.getItems({ pageIndex: 0, pageSize: 1 }, [id !== '' ? { name: 'ma_gd', value: id } : { name: '1', value: '1' }], { name: 'ma_gd', direction: 'asc' }).pipe(switchMap(item => {
  ...
  getFields(): Observable<Field[]> {
    ...
    return this.http.get<Field[]>(`assets/fields/lookup/dmmagd.json?r=${randomParam}`).pipe(
      switchMap(data => {
        data = data.map((item) => {
