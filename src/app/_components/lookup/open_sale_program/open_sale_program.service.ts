...
export class OpenSaleProgramService implements IGridService<OpenSaleProgram>{
  filter!: ItemFilter[];
  name?: string;
  ...
  getTitle(): string {
    return 'Danh mục chương trình mở bán';
  }
  getItems(page: { pageIndex: number; pageSize: number; }, filter: ItemFilter[], sort: ItemSort): Observable<Result<OpenSaleProgram>> {
    if (filter) {
      if (this.filter) {
    ...
    return this.http.post<Result<OpenSaleProgram>>(environment.apiUrl +
      `/Category/find/vdmctmoban?order_by=${order_by}&page_index=${page.pageIndex + 1}&page_size=${page.pageSize}`, body
    ).pipe();
  }
  getItem(id: string): Observable<OpenSaleProgram> {
    // return this.http.post<OpenSaleProgram>(environment.apiUrl +
  ...
  getFields(): Observable<Field[]> {
    ...
    return this.http.get<Field[]>(`assets/fields/lookup/vdmctmoban.json?r=${randomParam}`).pipe(
      switchMap(data => {
        data = data.map((item) => {
