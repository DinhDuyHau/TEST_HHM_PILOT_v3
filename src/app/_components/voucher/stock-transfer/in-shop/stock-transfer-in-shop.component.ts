...
const {
  MERCHANDISE_LIST
} = require('@assets/fields/grid/voucher-stock-transfer-in-shop.json');

@Component({
  selector: 'app-retail',
  templateUrl: './stock-transfer-in-shop.component.html',
...
export class StockTransferInShopComponent implements OnInit, AfterViewInit {
  ...
  entity = STOCK_TRANSFER_IN_SHOP_TICKET_ENTITY;
  ...
  constructor(
    ...
    private StockTransferInShopService: StockTransferInShopService,
    ...
    this.StockTransferInShopService.setTicket(this.ticket, this.option);
  }
  ...
  ngOnInit() {
    ...
    const getStatusList = () => {
      this.ticketApiService.getStatus([{ Name: 'ma_ct', Operator: '=', Value: STOCK_TRANSFER_IN_SHOP_TICKET_CODE }]).subscribe(result => {
      ...
      if (data.key) {
        this.ticketApiService.getVoucherByid(STOCK_TRANSFER_IN_SHOP_TICKET_ENTITY, data.key).subscribe((result) => {
          if (result.result) {
            ...
            this.StockTransferInShopService.loadData(result.result as any as VoucherDto);
        ...
        this.StockTransferInShopService.initTicket(this.ticket);
        getStatusList();
        this.tabIndexFocusFirst = this.tabIndex.imei;
      }
    });
  ...
  handleRemoveMerchandise(merchandise: Merchandise) {
    this.StockTransferInShopService.removeMerchandise(merchandise);
  }
  ...
  // Submit
  onSave() {
    const message = this.StockTransferInShopService.validateTicket(this.ticket);
    this.invalid = this.StockTransferInShopService.isInvalidForm(this.ticket.masterInfo);

    //check valid các trường số lượng và tiền trong grid hàng hóa và dịch vụ
    if (!this.StockTransferInShopService.isInvalidMerchandise(this.ticket.merchandise)) {
