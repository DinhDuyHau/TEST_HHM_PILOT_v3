...
import { MasterInfo, Receipt } from './event-gift.model';
...
export class EventGiftService implements IGridService<Receipt>{

  name?: string;
  voucher_type: TypeVoucher = VOUCHER_TYPE.EVENT_GIFT;
  ...
  getItems(page: { pageIndex: number; pageSize: number; }, filter: ItemFilter[], sort: ItemSort): Observable<Result<Receipt>> {
    ...
    if (!filter) {
      ...
      return this.http.post<Result<Receipt>>(environment.apiUrl +
        `/voucher/find/${this.voucher_type.sysid}?page_index=${page.pageIndex + 1}&page_size=${page.pageSize}${param}`, {}
      ).pipe();
    }
  }
