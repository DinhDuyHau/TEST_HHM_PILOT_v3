...
import { ReceiptDetail } from '../event-gift.model';
...
export class EventGiftDetailService implements IGridService<ReceiptDetail>{
  ...
  getTitle(): string {
    return 'Xuất hàng tặng theo sự kiện';
  }
  ...
  getFields(): Observable<Field[]> {
    ...
    return this.http.get<Field[]>(`assets/fields/grid/event-gift_detail.json?r=${randomParam}`).pipe(
      switchMap(data => {
        data = data.map((item) => {
