...
import { Receipt, ReceiptDetail } from '../return-supplier.model';
...
import { ReturnSupplierDetailService } from './return-supplier-detail.service';
...
export class ReturnSupplierDetailComponent extends Grid<ReceiptDetail> implements OnInit, AfterViewInit {
  // buttonsCustom!: Button[];
  override buttons = [];
  @ViewChild('form') form!: ElementRef;
  @ViewChild('btnSubmit') btnSubmit!: ElementRef;
  voucherForm!: FormGroup;
  title = 'Phiếu đề nghị trả lại nhà cung cấp';
  userName = '';
  ma_cuahang = '';
  ten_cuahang = '';
  item_code = '';
  ...
  submitButtonTitle = '';
  cancelButtonTitle = '';
  entity = VOUCHER_TYPE.RETURN_SUPPILER.sysid;
  ...
  constructor(
    ...
    private returnSupplier: ReturnSupplierService,
  ...
  ngOnInit() {
    ...
    if (this.mode == MODE.CREATE) {
      ...
      this.ticketApiService.getVoucherNumber('PRTran').subscribe(result => {
  ...
  async addItem(imei: string) {
    ...
    if (result.success && result.result) {
      ...
      if (this.data.masterInfo.ma_kh && this.data.masterInfo.ma_kh.trim() != response.ma_ncc.trim()) {
