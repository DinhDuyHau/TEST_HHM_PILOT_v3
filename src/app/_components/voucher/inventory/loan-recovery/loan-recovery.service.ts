...
import { MasterInfo, Receipt } from './loan-recovery.model';
...
export class LoanRecoveryService implements IGridService<Receipt>{
  ...
  delete(data: any): Observable<boolean> {
    ...
    if (stt_rec) {
      return this.http.delete<Result<MasterInfo>>(environment.apiUrl +
        '/voucher/delete/IRTran_PNM?voucherId=' + stt_rec
      ).pipe(map((res: any) => {
        if (res && res.success) {
  ...
  getTitle(): string {
    return 'Phiếu nhập hàng cho mượn';
  }
  getItems(page: { pageIndex: number; pageSize: number; }, filter: ItemFilter[], sort: ItemSort): Observable<Result<Receipt>> {
    ...
    if (!filter) {
      ...
      return this.http.post<Result<Receipt>>(environment.apiUrl +
        `/voucher/find/IRTran_PNM?page_index=${page.pageIndex + 1}&page_size=${page.pageSize}${param}`, {}
      ).pipe();
    }
  }
  getItem(id: string): Observable<Receipt> {
    ...
    return this.http.get<ResultDetailVoucher<Receipt>>(environment.apiUrl +
      `/voucher/getbyid/IRTran_PNM?id=${id}`
    ).pipe(map((res) => {
      return res.result;
    }));
  ...
  getFields(): Observable<Field[]> {
    ...
    return this.http.get<Field[]>(`assets/fields/grid/loan-recovery.json?r=${randomParam}`).pipe(
      switchMap(data => {
        data = data.map((item) => {
