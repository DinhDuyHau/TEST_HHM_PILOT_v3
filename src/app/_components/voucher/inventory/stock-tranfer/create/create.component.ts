...
import { Receipt, ReceiptDetail } from '../stock-tranfer.model';
...
import { StockTranferDetailService } from './stock-tranfer-detail.service';
...
export class StockTranferCreateComponent extends Grid<ReceiptDetail> implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    public stockTranferDetailService: StockTranferDetailService,
  ...
  initData(stt_rec: string) {
    this.stockTranferService.getItem(stt_rec).subscribe((item => {
  ...
  ngOnInit() {
    ...
    this.statusVoucher.getStatus(this.voucherCode).subscribe(result => {
      ...
      this.statusList = this.statusList.map((item) => {
        return { ...item, disabled: !item.right_yn && !item.xdefault };
      });
  ...
  onSubmit() {
    ...
    if (this.data.masterInfo.status == '0' || this.data.masterInfo.status == '2') {
      let all_imei: string[] = [];
