...
import { Receipt, ReceiptDetail } from '../internal-sale.model';
...
import { InternalSaleDetailService } from './internal-sale-detail.service';
...
export class InternalSaleCreateComponent extends Grid<ReceiptDetail> implements OnInit, AfterViewInit {
  ...
  disabled = false; // Khi view
  statusInit = ''; // Trạng thái khi bật form

  mode = 1;
  submitButtonTitle = '';
  cancelButtonTitle = '';
  flagDraft = true;
  // Điều kiện để tạo nháp hoá đơn -- Nếu đã có đủ imei trong chi tiết lúc mở phiếu thì mới tạo được, Nếu sửa imei thì cũng phải reset trạng thái, bắt buộc phải lưu phiếu trước khi sửa
  checkValidCreateDraft = true;

  // Danh sách imei cũ trước khi lưu phiếu
  ...
  constructor(
    ...
    private internalSaleService: InternalSaleService,
  ...
  clickButton(event: { buttonId: string; data?: any; }): void {
    ...
    if (event.buttonId == button.EditIMEIButton.id) {
      if (!event.data) {
        ...
        this.internalSaleDeatailService.openDialogIMEI(data).subscribe((value) => {
          if (value) {
  ...
  initData(stt_rec: string) {
    this.internalSaleService.getItem(stt_rec).subscribe((item => {
      item.masterInfo.ngay_ct = item.masterInfo.ngay_ct?.substring(0, 10);
      item.masterInfo.hddt_ngay_hd = item.masterInfo.hddt_ngay_hd?.substring(0, 10);
      item.masterInfo.hddt_ngay_ky = item.masterInfo.hddt_ngay_ky?.substring(0, 10);
      ...
      this.statusInit = item.masterInfo.status || '';

      if (item.masterInfo.status == '2') {
        this.disabled = true;
        //
      }
  ...
  onSubmit() {
    ...
    if (this.data.masterInfo.status == '2' && (!this.data.masterInfo.hddt_ma_tra_cuu || !this.data.masterInfo.hddt_so_hd || !this.data.masterInfo.hddt_so_seri || !this.data.masterInfo.hddt_ngay_hd)) {
      this.commonService.showMessageByName('lblWarningEInvoiceNotExist');
      return;
    }
    let all_imei: string[] = [];
  ...
  onCreateDraft() {
    // Kiểm tra xem tạo hoá đơn nháp đã hoàn thành chưa
    if (!this.flagDraft) {
      this.commonService.showMessageByName('operation_quickly');
      return;
    }
    if (this.data.masterInfo.status != '1') {
      this.commonService.showMessageByName('pxn_status_invalid_when_create_draft');
      return;
    }
    // // Kiểm tra xem imei đã bị quét lại chưa
    // const flag = this.checkImeiHasChange();
    // if (!this.checkValidCreateDraft) {
    //   this.commonService.showMessageByName('lbl_voucher_not_save');
    //   return;
    // }
    // if (!flag) {
    //   this.commonService.showMessageByName('lbl_save_voucher_when_change_imei');
    //   return;
    // }
    this.flagDraft = false;
    this.internalSaleDeatailService.createDraft(this.data).pipe(catchError((error: any) => {
      // xử lý lỗi ở đây
  ...
  onGetPublishedInv() {
    this.internalSaleDeatailService.getPublishedInv(this.data).subscribe((res: any) => {
      if (res.result.errorCode) {
        this.commonService.showMessage(res.result.description);
        return;
      }
  ...
  addIMEI(imei: string) {
    this.imeiService.getListImeiInfo([imei]).subscribe((res) => {
      ...
      //   this.commonService.addImeiToStorage(imei);
      //   //
      // });
      return true;
    });
  ...
  initCheckValidCreateDraft() {
    this.data.details[0].data.forEach((item: ReceiptDetail) => {
      if (item.ma_imei?.split(',').length !== item.so_luong) {
        this.checkValidCreateDraft = false;
        return;
      }
      if (item.ma_imei) {
        (item.ma_imei as string).split(',').forEach((item) => {
          this.list_imei_old.push(item.trim());
        });
      }
    });
  }
  ...
  checkImeiHasChange() {
    ...
    this.data.details[0].data.forEach((item: ReceiptDetail) => {
      if (item.ma_imei) {
        (item.ma_imei as string).split(',').forEach((item) => {
          if (this.list_imei_old[index].trim() != item.trim()) {
            flag = false;
            return;
          }
          index++;
