...
import { MasterInfo, Receipt } from './proposed-purchase.model';
...
export class ProposedPurchaseService implements IGridService<Receipt>{
  ...
  delete(data: any): Observable<boolean> {
    ...
    if (stt_rec) {
      return this.http.delete<Result<MasterInfo>>(environment.apiUrl +
        '/voucher/delete/PR3Tran?voucherId=' + stt_rec
      ).pipe(map((res: any) => {
        if (res && res.success) {
  ...
  getItems(page: { pageIndex: number; pageSize: number; }, filter: ItemFilter[], sort: ItemSort): Observable<Result<Receipt>> {
    ...
    if (!filter) {
      ...
      let param = `&ma_ct=PR3&ma_cuahang=${ma_cuahang}`;
      filter.forEach((item) => {
        param += `&${item.name}=${item.value}`;
      });
      return this.http.post<Result<Receipt>>(environment.apiUrl +
        `/voucher/find/PR3Tran?page_index=${page.pageIndex + 1}&page_size=${page.pageSize}${param}`, {}
      ).pipe();
    }
  }
  ...
  getFields(): Observable<Field[]> {
    ...
    return this.http.get<Field[]>(`assets/fields/grid/proposed-purchase.json?r=${randomParam}`).pipe(
      switchMap(data => {
        data = data.map((item) => {
