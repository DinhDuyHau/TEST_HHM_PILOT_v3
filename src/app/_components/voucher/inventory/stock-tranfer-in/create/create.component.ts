...
import { Receipt, ReceiptDetail } from '../stock-tranfer-in.model';
...
import { StockTranferInDetailService } from './stock-tranfer-in-detail.service';
...
@Component({
  selector: 'stock-tranfer-in-create',
  templateUrl: './create.component.html',
  styleUrls: ['./create.component.scss']
})
export class StockTranferInCreateComponent extends Grid<ReceiptDetail> implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    public stockTranferInDetailService: StockTranferInDetailService,
    ...
    private stockTranferInService: StockTranferInService,
    private statusVoucher: StatusVoucher,
    private el: ElementRef,
    private renderer: Renderer2
  ) {
    localStorage.setItem('useGridCached', '1');
    super(stockTranferInDetailService);
  ...
  clickButton(event: { buttonId: string; data?: any; }): void {
    ...
    if (event.buttonId == button.EditIMEIButton.id) {
      if (!event.data) {
        ...
        this.stockTranferInDetailService.openDialogIMEI(data).subscribe((value) => {
          if (value) {
  ...
  initData(stt_rec: string) {
    this.stockTranferInService.getItem(stt_rec).subscribe((item => {
    ...
    if (imeiInvalid.length) {
      ...
      this.stockTranferInService.update(this.data).subscribe((item: any) => {
        ...
        if (item.success) {
          ...
          this.commonService.handleResponseErrorVoucher(item, 'voucher/stock-tranfer-in');
        }
      });
    }
  }
  ...
  onCancel() {
    this.router.navigate(['voucher/stock-tranfer-in']);
  }
  ...
  addIMEI(imei: string) {
    this.imeiService.getListImeiInfo([imei]).subscribe((res) => {
      ...
      // this.imeiService.setUpSaleOrder([imei], true, 1, this.voucherCode).subscribe((res) => {
