...
import { WithDrawDetail, WithDraw } from '../withdraw-baokim.model';
...
import { WithdrawBaokimDetailService } from './withdraw-baokim-detail.service';
...
export class WithdrawBaokimDetailComponent extends Grid<WithDrawDetail> implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    public WithdrawBaokimDetailService: WithdrawBaokimDetailService,
    ...
    private WithDrawBaokimService: WithDrawBaokimService,
    ...
    super(WithdrawBaokimDetailService);
  ...
  initData(stt_rec: string) {
    this.WithDrawBaokimService.getItem(stt_rec).subscribe((item => {
  ...
  ngOnInit() {
    ...
    if (this.mode == MODE.CREATE) {
      ...
      this.ticketApiService.getVoucherNumber('CRTran_PCG').subscribe(result => {
  ...
  onSubmit() {
    ...
    if (this.mode == MODE.UPDATE) {
      ...
      this.WithDrawBaokimService.update(this.data).subscribe((item: any) => {
        this.loading = false;
      ...
      this.isDisabled = true;
      this.data.details[1].data = this.paymentServiceShop.convertPaymentToRequest(this.payment, this.data.masterInfo);
      this.WithDrawBaokimService.create(this.data).subscribe((item: any) => {
        this.loading = false;
        this.isDisabled = false;
        if (item.success) {
          this.router.navigate(['..'], { relativeTo: this.route });
