...
import { Receipt, ReceiptDetail } from '../close-shift-payment.model';
...
import { CloseShiftPaymentDetailService } from './close-shift-payment-detail.service';
...
export class CloseShiftPaymentDetailComponent extends Grid<ReceiptDetail> implements OnInit, AfterViewInit {
  ...
  cancelButtonTitle = '';
  [key: string]: any;
  entity = VOUCHER_TYPE.CLOSE_SHIFT_PAYMENT.sysid;

  override gridType = GridType.GridDetail;
  actionButtons = [button.DeleteButton];
  constructor(
    private formBuilder: FormBuilder,
    public CloseShiftPaymentDetailService: CloseShiftPaymentDetailService,
    ...
    private CloseShiftPayment: CloseShiftPaymentService,
    ...
    super(CloseShiftPaymentDetailService);
  ...
  initData(stt_rec: string) {
    this.CloseShiftPayment.getItem(stt_rec).subscribe((item => {
  ...
  onSubmit() {
    ...
    if (this.mode == MODE.UPDATE) {
      this.CloseShiftPayment.update(this.data).subscribe((item: any) => {
        this.loading = false;
      ...
      this.isDisabled = true;
      this.CloseShiftPayment.create(this.data).subscribe((item: any) => {
        this.loading = false;
        this.isDisabled = false;
        if (item.success) {
          this.router.navigate(['..'], { relativeTo: this.route });
