...
import { MasterInfo, Receipt } from './refund-qrbanking.model';
...
export class RefundQrBankingService implements IGridService<Receipt> {
  ...
  delete(data: any): Observable<boolean> {
    ...
    if (stt_rec) {
      return this.http.delete<Result<MasterInfo>>(environment.apiUrl +
        '/voucher/delete/CDTran_HT1?voucherId=' + stt_rec
      ).pipe(map((res: any) => {
        if (res && res.success) {
  ...
  getItems(page: { pageIndex: number; pageSize: number; }, filter: ItemFilter[], sort: ItemSort): Observable<Result<Receipt>> {
    ...
    if (!filter) {
      ...
      let param = `&ma_ct=PCC&ma_cuahang=${ma_cuahang}`;
      filter.forEach((item) => {
        param += `&${item.name}=${item.value}`;
      });
      return this.http.post<Result<Receipt>>(environment.apiUrl +
        `/voucher/find/CDTran_HT1?page_index=${page.pageIndex + 1}&page_size=${page.pageSize}${param}`, {}
      ).pipe();
    }
  }
  getItem(id: string): Observable<Receipt> {
    ...
    return this.http.get<ResultDetailVoucher<Receipt>>(environment.apiUrl +
      `/voucher/getbyid/CDTran_HT1?id=${id}`
    ).pipe(map((res) => {
      return res.result;
    }));
  ...
  getFields(): Observable<Field[]> {
    ...
    return this.http.get<Field[]>(`assets/fields/grid/close-shift-payment.json?r=${randomParam}`).pipe(
      switchMap(data => {
        data = data.map((item) => {
