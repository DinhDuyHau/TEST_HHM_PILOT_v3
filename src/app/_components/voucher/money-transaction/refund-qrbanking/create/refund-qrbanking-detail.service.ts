...
import { ReceiptDetail } from '../refund-qrbanking.model';
...
export class RefundQrBankingDetailService implements IGridService<ReceiptDetail> {
  ...
  getTitle(): string {
    return 'Phiếu đề nghị hoàn tiền thanh toán QR';
  }
  ...
  getFields(): Observable<Field[]> {
    ...
    return this.http.get<Field[]>(`assets/fields/grid/close-shift-payment_detail.json?r=${randomParam}`).pipe(
      switchMap(data => {
        data = data.map((item) => {
