...
import { DepositDetail, Deposit } from '../deposit-baokim.model';
...
import { DepositBaokimDetailService } from './deposit-baokim-detail.service';
...
export class DepositBaokimDetailComponent extends Grid<DepositDetail> implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    public DepositBaokimDetailService: DepositBaokimDetailService,
    ...
    private DepositBaokimService: DepositBaokimService,
    ...
    super(DepositBaokimDetailService);
  ...
  initData(stt_rec: string) {
    this.DepositBaokimService.getItem(stt_rec).subscribe((item => {
  ...
  ngOnInit() {
    ...
    if (this.mode == MODE.CREATE) {
      ...
      this.ticketApiService.getVoucherNumber('CRTran_PTG').subscribe(result => {
  ...
  onSubmit() {
    ...
    if (this.mode == MODE.UPDATE) {
      ...
      this.DepositBaokimService.update(this.data).subscribe((item: any) => {
        this.loading = false;
      ...
      this.isDisabled = true;
      this.data.details[1].data = this.paymentServiceShop.convertPaymentToRequest(this.payment, this.data.masterInfo);
      this.DepositBaokimService.create(this.data).subscribe((item: any) => {
        this.loading = false;
        this.isDisabled = false;
        if (item.success) {
          this.router.navigate(['..'], { relativeTo: this.route });
