...
import { MasterInfo, Receipt } from './deposist-return-payment.model';
...
export class DeposistReturnReceiptService implements IGridService<Receipt>{
  ...
  delete(data: any): Observable<boolean> {
    ...
    if (stt_rec) {
      return this.http.delete<Result<MasterInfo>>(environment.apiUrl +
        '/voucher/delete/CDTran_PCH?voucherId=' + stt_rec
      ).pipe(map((res: any) => {
        if (res && res.success) {
  ...
  getItems(page: { pageIndex: number; pageSize: number; }, filter: ItemFilter[], sort: ItemSort): Observable<Result<Receipt>> {
    ...
    if (!filter) {
      ...
      let param = `&ma_ct=PCH&ma_cuahang=${ma_cuahang}`;
      filter.forEach((item) => {
        param += `&${item.name}=${item.value}`;
      });
      return this.http.post<Result<Receipt>>(environment.apiUrl +
        `/voucher/find/CDTran_PCH?page_index=${page.pageIndex + 1}&page_size=${page.pageSize}${param}`, {}
      ).pipe();
    }
  }
  getItem(id: string): Observable<Receipt> {
    ...
    return this.http.get<ResultDetailVoucher<Receipt>>(environment.apiUrl +
      `/voucher/getbyid/CDTran_PCH?id=${id}`
    ).pipe(map((res) => {
      return res.result;
    }));
  ...
  getFields(): Observable<Field[]> {
    ...
    return this.http.get<Field[]>(`assets/fields/grid/deposist-return-payment.json?r=${randomParam}`).pipe(
      switchMap(data => {
        data = data.map((item) => {
