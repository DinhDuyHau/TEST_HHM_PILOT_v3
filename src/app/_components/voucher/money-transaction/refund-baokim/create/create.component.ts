...
import { RefundDetail, Refund } from '../refund-baokim.model';
import { Grid, GridType } from '@app/_components/gridV2/grid.model';
...
import { RefundBaokimDetailService } from './refund-baokim-detail.service';
...
export class RefundBaokimDetailComponent extends Grid<RefundDetail> implements OnInit, AfterViewInit {
  ...
  [key: string]: any;
  entity = VOUCHER_TYPE.REFUND_BAOKIM.sysid;
  ...
  constructor(
    ...
    public RefundBaokimDetailService: RefundBaokimDetailService,
    ...
    private RefundBaokimService: RefundBaokimService,
    ...
    super(RefundBaokimDetailService);
  ...
  initData(stt_rec: string) {
    this.RefundBaokimService.getItem(stt_rec).subscribe((item => {
      ...
      this.voucherForm = this.formBuilder.group({
        ...
        ghi_chu: [this.data.masterInfo.ghi_chu],
        ...
        dien_giai_ct_goc: [this.data.masterInfo.dien_giai_ct_goc, Validators.required],
      });
      this.stockService.setItemFilter([{ name: 'ma_cuahang', value: this.data.masterInfo.ma_cuahang }, { name: 'ma_loai', value: 'HM' }]);
      this.dataSource = new MatTableDataSource<RefundDetail>(this.data.details[0].data);
  ...
  ngOnInit() {
    ...
    if (this.mode == MODE.CREATE) {
      ...
      this.ticketApiService.getVoucherNumber('CDTran_PCL').subscribe(result => {
  ...
  onSubmit() {
    ...
    const ignoreFields = ['dien_giai_ct_goc']; // các field muốn bỏ qua
    if (this.voucherForm.invalid) {
      Object.keys(this.voucherForm.controls).forEach(key => {
        if (ignoreFields.includes(key)) {
          return; // bỏ qua field chỉ định
        }
    ...
    if (this.mode == MODE.UPDATE) {
      this.RefundBaokimService.update(this.data).subscribe((item: any) => {
        this.loading = false;
      ...
      this.RefundBaokimService.create(this.data).subscribe((item: any) => {
        this.loading = false;
        this.isDisabled = false;
        if (item.success) {
          this.router.navigate(['..'], { relativeTo: this.route });
  ...
  handleInputChange(controlName: string, $event: any): void {
    ...
    if (controlName === 'dien_giai_ct_goc' && $event.length > 250) {
      return this.commonService.showMessage('Không được quá 250 ký tự');
    }
  ...
  onCancel() {
    this.router.navigate(['voucher/refund-baokim']);
  }
  onEnter(event: any) {
