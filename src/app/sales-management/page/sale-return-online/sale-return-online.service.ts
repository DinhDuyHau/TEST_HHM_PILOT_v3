...
import { MasterInfo, Merchandise, ReturnSaleOnlineTicketCreate, SaleReturnServiceOnlineModel, TAB_NAME } from '@app/sales-management/model/ticket/sale-return-online/model';
import { CommonService } from '../common/common.service';
import { MerchandiseService } from '../common/merchandise.service';
import { MerchandiseRequest, MasterInfoRequest, SaleReturnServiceOnlineRequest } from '@app/sales-management/model/ticket/sale-return-online/request.model';
...
export class SaleReturnOnlineService {
    ...
    prepareVoucher(): VoucherDto {
        ...
        voucherDto.details = [...voucherDto.details, { id: 4, name: TAB_NAME.SERVICE, data: this.serviceOfMerchandiseService.convertServiceToRequest(this.ticket.service, voucherDto.masterInfo, SaleReturnServiceOnlineRequest) }];
        return voucherDto;
    }
    ...
    initTicket(ticket: ReturnSaleOnlineTicketCreate) {
        ...
        this.ticketApiService.getVoucherNumber(TICKET_ENTITY.RETURN_ONLINE).subscribe(result => {
            ...
            if (current_soct === newSoCT) {
                ...
                this.commonService.saveVoucherNumberLocalStorage(ticket.masterInfo.so_ct, TICKET_CODE.RETURN_ONLINE);
            }
        });
    ...
    convertFromVoucherService(src: any[], des: any[], TCreator?: { new(): any; }) {
        const rs = src.map((e: any, i: number) => {
            let serviceNew = new SaleReturnServiceOnlineModel();
            if (TCreator) {
