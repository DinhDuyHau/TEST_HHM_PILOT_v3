...
import { MasterInfo, Merchandise, SaleOnlineEcommerceTicket, TAB_NAME } from '@app/sales-management/model/ticket/sale-online-ecommerce/model';
...
import { MerchandiseRequest, MasterInfoRequest } from '@app/sales-management/model/ticket/sale-online-ecommerce/request.model';
...
export class SaleOnlineEcommerceService {
    discountOptions: Discount[] = [];
    isNeedCalcDiscount = true;
    ticket!: SaleOnlineEcommerceTicket;
    option!: Option;
    ...
    //#region setter
    setTicket(ticket: SaleOnlineEcommerceTicket, option: Option) {
        this.ticket = ticket;
        this.option = option;
    }
    ...
    initTicket(ticket: SaleOnlineEcommerceTicket) {
        const userJson = localStorage.getItem('user');
        const userObj = userJson !== null && JSON.parse(userJson);

        ticket.masterInfo.ma_ct = TICKET_CODE.ONLINE_ECOMMERCE;
        ...
        this.ticketApiService.getVoucherNumber(TICKET_ENTITY.ONLINE_ECOMMERCE).subscribe(result => {
            ...
            if (current_soct === newSoCT) {
                ...
                this.commonService.saveVoucherNumberLocalStorage(ticket.masterInfo.so_ct, TICKET_ENTITY.ONLINE_ECOMMERCE);
            }
        });
    }
    ...
    // handle select no_km
    onChangePromotionalDebt(event: { item: Merchandise, index: number, checked: boolean, columnName: string }, ticket: SaleOnlineEcommerceTicket) {
    ...
    validateTicket(ticket: SaleOnlineEcommerceTicket): string {
        let message = '';
