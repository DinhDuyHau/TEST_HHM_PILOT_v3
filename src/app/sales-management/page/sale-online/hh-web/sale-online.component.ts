...
export class SaleOnlineComponent implements OnInit, AfterViewInit {
    ...
    openSearchTelesaleDialog() {
        this.commonService.openDialog(SearchDialogComponent,
            { keyword: '', componentName: SEARCH_COMPONENT_NAME.TELESALE_SEARCH }, 'search-style-dialog')
            .afterClosed()
            .subscribe(result => {
                if (result)
    ...
    */
    ...
    handleChangeVoucherCode($event: any) {
        ...
        this.saleOnlineService.voucherCheck(this.voucher_code, member, phone, stock, skus).subscribe({
            next: result => {
    ...
    addVoucherCode(validSkus: string[], inputResponse: any) {
        ...
        this.saleOnlineService.getDiscountVoucherCode(ngay_ct).subscribe(result => {
            const response = result as any;
            ...
            if (response.success && response.result != null) {
                ...
                this.saleOnlineService.calcMoney(); // sau khi thêm chiết khấu thành công mới tính lại tiền
