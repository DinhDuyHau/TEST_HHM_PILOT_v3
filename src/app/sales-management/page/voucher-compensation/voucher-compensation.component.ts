...
const {
  DISCOUNT_LIST,
  GUARANTEE_LIST,
  MERCHANDISE_COMPENSATION_LIST,
  SERVICE_LIST,
  PACKAGE_LIST
} = require('@assets/fields/grid/sales-fields-table.json');

@Component({
  selector: 'voucher-compensation',
  templateUrl: './voucher-compensation.component.html',
  styleUrls: ['./voucher-compensation.component.scss'],
})
export class VoucherCompensationComponent implements OnInit, AfterViewInit {
  ...
  uploading = true;
  merchandiseColumns = MERCHANDISE_COMPENSATION_LIST;
  ...
  constructor(
    ...
    private voucherCompensationService: VoucherCompensationService,
  ...
  ngOnInit() {
    ...
    this.route.queryParams.pipe().subscribe((data: any) => {
      if (data.key) {
        this.ticketApiService.getVoucherByid(TICKET_ENTITY.STOCK_COMPENSATION, data.key).subscribe((result) => {
          if (result.result) {
            ...
            if (this.mode === MODE.UPDATE &&
              !((result.result as any).masterInfo.status === STATUS_LIST.STOCK_COMPENSATION.CREATE
                || (result.result as any).masterInfo.status === STATUS_LIST.STOCK_COMPENSATION.PENDING_PAYMENT
                || (result.result as any).masterInfo.status === STATUS_LIST.STOCK_COMPENSATION.PENDING_PUBLISH
              )) {
            ...
            this.voucherCompensationService.loadData(result.result as any as VoucherDto, (image: any) => { image && this.getImageCustomerFile(image); });
            ...
            this.voucherCompensationService.getConversionPoint().subscribe(result => {
        ...
        this.voucherCompensationService.initTicket(this.ticket);
        this.getStatusList();
  ...
  getStatusList = () => {
    this.ticketApiService.getStatusWithOrder([{ Name: 'ma_ct', Operator: '=', Value: TICKET_CODE.STOCK_COMPENSATION }], 'xorder,status').subscribe(result => {
  ...
  handleGetDeposit() {
    this.voucherCompensationService.getDeposit().subscribe((result: any) => {
  ...
  onEnterCustomerCode(ma_kh: string) {
    this.customerApiService.getOneById(ma_kh).subscribe(result => {
      if (result.success && result.result) {
        ...
        this.voucherCompensationService.resetCustomerInfo(this.ticket);
  ...
  openAddCustomerDialog(ma_kh = ''): void {
    this.commonService.openDialog(CustomerCreateDialogComponent, { ma_kh: ma_kh }, 'fullscreen-dialog')
      ...
      .subscribe((customer: Customer) => {
        customer && this.voucherCompensationService.setInfoCustomer(customer);
      });
  }
  //#endregion
  ...
  handleAddGuarantee(merchandiseResponse: any) {
    this.guaranteeApiService.getOneById(TICKET_ENTITY.STOCK_COMPENSATION, { ma_vt: merchandiseResponse.ma_vt, ma_kho: merchandiseResponse.ma_kho }).subscribe(result => {
      if (result && result.success && result?.result?.length) {
        const guarantee = result.result[0];
        this.voucherCompensationService.addGuaranteeMerchandise(merchandiseResponse, guarantee);
      }
    });
  }
  // #endregion Guarantee
    ...
    if (!merchandise) {
      ...
      if (merchandiseResponse.promotions && merchandiseResponse.promotions.length) {
        ...
        if (discount) {
          ...
          this.voucherCompensationService.addPromotionMerchandise(discount, merchandiseResponse.ma_imei);
        }
      }
      */
  ...
  onEnterImeiCode(ma_imei: string) {
    ...
    this.voucherCompensationService.getImeiInStore(ma_imei).subscribe(result => {
  ...
  handleRemoveMerchandise(merchandise: Merchandise) {
    if (merchandise.km_yn) {
      ...
      this.voucherCompensationService.removeMerchandise(merchandise);
  ...
  onChangePromotionalDebt(event: { item: Merchandise, index: number, checked: boolean, columnName: string }) {
    // this.voucherCompensationService.onChangePromotionalDebt(event, this.ticket);
  }
  ...
  openCalcDiscountDialog(isGridItem: boolean = false, event: { item: Merchandise } | null = null, loai_ck: string = '') {
      ...
      this.commonService.openDialog(DiscountSelectComponent, { dataSource: dataSource, currentItem: currentItem })
        .afterClosed().subscribe(discountSelected => {
          if (discountSelected) {
            ...
            // this.voucherCompensationService.addDiscount(discountAdded);
            // this.voucherCompensationService.removeDiscount(disocuntRemoved);
            this.voucherCompensationService.updateDiscount(discountSelected, isGridItem, currentRow ? currentRow!.item : null);
          }
        });
    };
    ...
    const rs = this.voucherCompensationService.calcDiscount(loai_ck);
    if (rs) {
      rs.subscribe(result => {
        if (result.success) {
          ...
          // this.voucherCompensationService.removeDiscount(discountsInvalid);
  ...
  onRemoveDiscount(event: { item: Discount }) {
    ...
    const rs = this.voucherCompensationService.calcDiscount();
    if (rs) {
      rs.subscribe(result => {
        if (result.success) {
          this.discountCanApply = this.discountService.convertDiscountFromList(result.result as any);
          ...
          this.voucherCompensationService.updateDiscount(discountAfterRemove, false, null, true);
        }
        else {
    ...
    // this.voucherCompensationService.updateDiscount(this.ticket.discount.filter(x => x.ma_ck !== event.item.ma_ck));
    // this.voucherCompensationService.removeDiscount([event.item]);
    // this.voucherCompensationService.calcMoney();
  }
  // #endregion discount
  ...
  onAddService(event: { item: Merchandise }) {
    // this.voucherCompensationService.addServiceForMerchandise(event.item, this.ticket);
  }

  // click button add service
  onRemoveService(event: { item: Service }) {
    if (event.item.km_yn) {
      this.voucherCompensationService.removePromotionService(event.item);
    } else {
      this.voucherCompensationService.removeService(event.item, this.ticket);
    }
  }
  // #endregion service
  ...
  onAddPackage(event: { item: Merchandise }) {
    // this.voucherCompensationService.addPackageForMerchandise(event.item, this.ticket)
  }

  onRemovePackage(event: { item: Package }) {
    this.voucherCompensationService.removePackage(event.item, this.ticket)
  }
  // #endregion package
  ...
  onSave() {
    ...
    if (!this.voucherCompensationService.isInvalidMerchandise(this.ticket.merchandise)) {
      this.commonService.showMessage(Language.content.grid_merchandise_invalid);
      return;
    }
    if (!this.voucherCompensationService.isInvalidService(this.ticket.service)) {
        ...
        if (this.mode === MODE.UPDATE && !this.isSaving) {
          ...
          this.ticketApiService.updateVoucher(TICKET_ENTITY.STOCK_COMPENSATION, voucherDto).subscribe(result => {
            ...
            if (result.success) {
              ...
              this.commonService.handleResponseErrorVoucher(result, 'voucher/compensation');
            }
          });
  ...
  handleProcessImei(ma_imei: string) {
    ...
    this.voucherCompensationService.getImeiInStore(this.ma_imei).subscribe(result => {
      if (result.success && result.result.length) {
