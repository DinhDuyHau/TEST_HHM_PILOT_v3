...
import { MasterInfo, Merchandise, ItinerantSaleTicket, TAB_NAME } from '@app/sales-management/model/ticket/sale-itinerant/model';
...
import { MerchandiseRequest, MasterInfoRequest } from '@app/sales-management/model/ticket/sale-itinerant/request.model';
...
export class SaleItinerantService {
    discountOptions: Discount[] = [];
    isNeedCalcDiscount = true;
    ticket!: ItinerantSaleTicket;
    option!: Option;
    constructor(
    ...
    //#region setter
    setTicket(ticket: ItinerantSaleTicket, option: Option) {
        this.ticket = ticket;
        this.option = option;
    }
    ...
    prepareVoucher(): VoucherDto {
        ...
        voucherDto.details = [...voucherDto.details, { id: 6, name: TAB_NAME.PACKAGE, data: this.packageOfMerchandiseService.convertPackageToRequest(this.ticket.packages, voucherDto.masterInfo, PackageRequest) }];

        return voucherDto;
    }

    initTicket(ticket: ItinerantSaleTicket) {
        const userJson = localStorage.getItem('user');
        const userObj = userJson !== null && JSON.parse(userJson);
        ticket.masterInfo.ma_ct = TICKET_CODE.ITINERANT;
        ...
        this.ticketApiService.getVoucherNumber(TICKET_ENTITY.ITINERANT).subscribe(result => {
            ...
            if (current_soct === newSoCT) {
                ...
                this.commonService.saveVoucherNumberLocalStorage(ticket.masterInfo.so_ct, TICKET_CODE.ITINERANT);
            }
        });
    ...
    //#region project
    setInfoProject(project: any) {
        this.ticket.masterInfo.job_id = project.ma_vv;
        this.ticket.masterInfo.ten_vv = project.ten_vv;
    }
    ...
    resetCustomerInfo(ticket: ItinerantSaleTicket) {
    ...
    getImeiInStore(imei: string, ngay_ct: Date | null = null) {
        return this.imeiApiService.getImeiInStore(imei, this.ticket.masterInfo.ma_cuahang, TICKET_CODE.ITINERANT, ngay_ct);
    }
    ...
    // handle select no_km
    onChangePromotionalDebt(event: { item: Merchandise, index: number, checked: boolean, columnName: string }, ticket: ItinerantSaleTicket) {
    ...
    calcDiscount(): Observable<ResultNoPaging<Discount>> | undefined {
        ...
        if (this.isNeedCalcDiscount) {
            return this.discountApiService.getDiscountForTicket(entity, merchandise, ma_cuahang, ma_kh, ngay_lap, service, '', TICKET_CODE.ITINERANT, stt_rec);
        }
        return;
    }
    ...
    // #region service
    addServiceForMerchandise(item: Merchandise, ticket: ItinerantSaleTicket) {
    ...
    // Remove service
    removeService(item: Service, ticket: ItinerantSaleTicket) {
    ...
    //#region package
    addPackageForMerchandise(item: Merchandise, ticket: ItinerantSaleTicket) {
    ...
    validateTicket(ticket: ItinerantSaleTicket): string {
        let message = '';
        ...
        } else if (!ticket.masterInfo.job_id) {
            message = this.commonService.getMessage('lbl_invalid_ma_da');
