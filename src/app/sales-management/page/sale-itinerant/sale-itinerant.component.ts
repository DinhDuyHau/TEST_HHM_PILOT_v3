...
import { SaleItinerantService } from './sale-itinerant.service';
import { Merchandise, ItinerantSaleTicket } from '@app/sales-management/model/ticket/sale-itinerant/model';
...
@Component({
  selector: 'app-sale-itinerant',
  templateUrl: './sale-itinerant.component.html',
  styleUrls: ['./sale-itinerant.component.scss'],
})
export class SaleItinerantComponent implements OnInit, AfterViewInit {
  ticket: ItinerantSaleTicket = new ItinerantSaleTicket;
  ...
  constructor(
    ...
    private saleItinerantService: SaleItinerantService,
  ...
  handleGetDeposit() {
    this.saleItinerantService.getDeposit().subscribe((result: any) => {
  ...
  onEnterProjectCode(ma_vv: string) {
    //
  }

  openSearchProjectDialog() {
    this.commonService.openDialog(SearchDialogComponent,
      { keyword: '', componentName: SEARCH_COMPONENT_NAME.ITINERANT }, 'search-style-dialog')
      .afterClosed().subscribe(result => {
        if (result) {
          this.saleItinerantService.setInfoProject(result);
        }
      });
  }
  ...
  openAddCustomerDialog(ma_kh = ''): void {
    this.commonService.openDialog(CustomerCreateDialogComponent, { ma_kh: ma_kh, addOrUpdate: this.addOrUpdateCustomer }, 'fullscreen-dialog')
      .afterClosed()
      .subscribe((customer: Customer) => {
        customer && this.saleItinerantService.setInfoCustomer(customer);
      });
  }
  //#endregion
  ...
  handleAddGuarantee(merchandiseResponse: any) {
    this.guaranteeApiService.getOneById(TICKET_ENTITY.RETAIL, { ma_vt: merchandiseResponse.ma_vt, ma_kho: merchandiseResponse.ma_kho }).subscribe(result => {
      if (result && result.success && result?.result?.length) {
        ...
        this.saleItinerantService.addGuaranteeMerchandise(merchandiseResponse, guarantee);
      }
    });
  }
  ...
  handleRemoveMerchandise(merchandise: Merchandise) {
    if (merchandise.km_yn) {
      this.saleItinerantService.removePromotionMechandise(merchandise);
    } else {
      this.saleItinerantService.removeMerchandise(merchandise);
  ...
  onChangePromotionalDebt(event: { item: Merchandise, index: number, checked: boolean, columnName: string }) {
    this.saleItinerantService.onChangePromotionalDebt(event, this.ticket);
  }
  ...
  openCalcPackageDialog() {
    ...
    if (this.merchandiseSelected.ma_imei) {
      this.saleItinerantService.addServiceForMerchandise(this.merchandiseSelected, this.ticket);
    }
  }
  // #endregion package
    ...
    const openDialog = (dataSource: Discount[], currentItem: Discount[]) => {
      this.commonService.openDialog(DiscountSelectComponent, { dataSource: dataSource, currentItem: currentItem })
        .afterClosed().subscribe(discountSelected => {
          if (discountSelected) {
            this.saleItinerantService.updateDiscount(discountSelected);
          }
        });
    };

    const discountCurrent = this.discountService.getDiscountCurrent(this.ticket.discount);
    const rs = this.saleItinerantService.calcDiscount();
    if (rs) {
      rs.subscribe(result => {
        if (result.success) {
    ...
    if (rs) {
      rs.subscribe(result => {
        if (result.success) {
          ...
          this.saleItinerantService.updateDiscount(discountAfterRemove);
        }
        else {
  ...
  onAddService(event: { item: Merchandise }) {
    this.saleItinerantService.addServiceForMerchandise(event.item, this.ticket);
  }

  // click button add service
  onRemoveService(event: { item: Service }) {
    if (event.item.km_yn) {
      this.saleItinerantService.removePromotionService(event.item);
    } else {
      this.saleItinerantService.removeService(event.item, this.ticket);
    }
  }
  // #endregion service
  ...
  onAddPackage(event: { item: Merchandise }) {
    this.saleItinerantService.addPackageForMerchandise(event.item, this.ticket)
  }

  onRemovePackage(event: { item: Package }) {
    this.saleItinerantService.removePackage(event.item, this.ticket)
  }
  // #endregion package
  ...
  onSave() {
    ...
    if (message) {
      ...
      const voucherDto = this.saleItinerantService.prepareVoucher();
