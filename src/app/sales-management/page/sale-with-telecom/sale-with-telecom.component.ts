...
import { SaleWithTelecomService } from './sale-with-telecom.service';
import { Merchandise, TelecomSaleTicket } from '@app/sales-management/model/ticket/sale-with-telecom/model';
...
@Component({
  ...
  templateUrl: './sale-with-telecom.component.html',
  styleUrls: ['./sale-with-telecom.component.scss'],
})
export class SaleWithTelecomComponent implements OnInit, AfterViewInit {
  ticket: TelecomSaleTicket = new TelecomSaleTicket;
  ...
  uploading = true;
  merchandiseColumns = MERCHANDISE_TELECOM_LIST;
  ...
  merchandiseSelected: Merchandise = new Merchandise;
  previewImage = '';
  depositCanApply: any[] = [];
  ...
  constructor(
    ...
    private saleWithTelecomService: SaleWithTelecomService,
  ...
  handleAddCustomer(customer: Customer) {
    ...
    this.saleWithTelecomService.removeDiscountForCustomer();
    this.saleWithTelecomService.setInfoCustomer(customer);
    this.handleGetDeposit();
    this.saleWithTelecomService.setIsNeedCalcDiscount(true);
    this.discountService.resetDiscount(this.ticket.discount);
    this.saleWithTelecomService.calcMoney();
    this.saleWithTelecomService.getConversionPoint().subscribe(result => {
      if (result && result.success && result.result !== null) {
        this.conversionPoints = result.result;
        this.ticket.payment.sd_diem.diem_qd = result.result;
  ...
  handleGetDeposit() {
    this.saleWithTelecomService.getDeposit().subscribe((result: any) => {
  ...
  handleAddGuarantee(merchandiseResponse: any) {
    this.guaranteeApiService.getOneById(TICKET_ENTITY.RETAIL, { ma_vt: merchandiseResponse.ma_vt, ma_kho: merchandiseResponse.ma_kho }).subscribe(result => {
      if (result && result.success && result?.result?.length) {
        ...
        this.saleWithTelecomService.addGuaranteeMerchandise(merchandiseResponse, guarantee);
      }
    });
  }
  // #endregion Guarantee
    ...
    if (!merchandise) {
      ...
      if (merchandiseResponse.promotions && merchandiseResponse.promotions.length) {
        ...
        if (discount) {
          ...
          this.saleWithTelecomService.addPromotionMerchandise(discount, merchandiseResponse.ma_imei);
        }
      }
  ...
  onEnterImeiCode(ma_imei: string) {
    ...
    this.saleWithTelecomService.getImeiInStore(ma_imei, ngay_ct).subscribe(result => {
  ...
  onRemoveMerchandise(event: { item: Merchandise }) {
    const { ma_imei } = this.merchandiseSelected;
    if (ma_imei && event.item.ma_imei == ma_imei) {
      this.merchandiseSelected.ma_imei = '';
      this.disabledSelectPackage = false;
    }
  ...
  handleRemoveMerchandise(merchandise: Merchandise) {
    if (merchandise.km_yn) {
      this.saleWithTelecomService.removePromotionMechandise(merchandise);
    } else {
      this.saleWithTelecomService.removeMerchandise(merchandise);
  ...
  onChangePromotionalDebt(event: { item: Merchandise, index: number, checked: boolean, columnName: string }) {
    this.saleWithTelecomService.onChangePromotionalDebt(event, this.ticket);
  }
    ...
    if (ma_imei) {
      ...
      this.saleWithTelecomService.getImeiInStore(ma_imei, ngay_ct).subscribe(result => {
        if (result && result.result && result.result.length) {
          ...
          if (typeof nh_vt5 === 'string' && nh_vt5.trim() === 'SIM') {
            this.disabledSelectPackage = false;
          }
        }
      });
  ...
  onSelectMerchandise(event: any) {
    this.merchandiseSelected.ma_imei = event.item.ma_imei;
    this.handleCheckStatusButtonPackage(this.merchandiseSelected.ma_imei);
  }

  openCalcPackageDialog() {
    this.handleCheckStatusButtonPackage(this.merchandiseSelected.ma_imei);
    if (this.merchandiseSelected.ma_imei) {
      this.saleWithTelecomService.addServiceForMerchandise(this.merchandiseSelected, this.ticket);
    }
  }
  //#endregion package
    ...
    const openDialog = (dataSource: Discount[], currentItem: Discount[]) => {
      this.commonService.openDialog(DiscountSelectComponent, { dataSource: dataSource, currentItem: currentItem })
        .afterClosed().subscribe(discountSelected => {
          if (discountSelected) {
            this.saleWithTelecomService.updateDiscount(discountSelected);
          }
        });
    };

    const discountCurrent = this.discountService.getDiscountCurrent(this.ticket.discount);
    const rs = this.saleWithTelecomService.calcDiscount();
    if (rs) {
      rs.subscribe(result => {
        if (result.success) {
    ...
    if (rs) {
      rs.subscribe(result => {
        if (result.success) {
          ...
          this.saleWithTelecomService.updateDiscount(discountAfterRemove);
        }
        else {
  ...
  onAddService(event: { item: Merchandise }) {
    this.saleWithTelecomService.addServiceForMerchandise(event.item, this.ticket);
  }

  // click button add service
  onRemoveService(event: { item: Service }) {
    if (event.item.km_yn) {
      this.saleWithTelecomService.removePromotionService(event.item);
    } else {
      this.saleWithTelecomService.removeService(event.item, this.ticket);
    }
  }
  // #endregion service
  ...
  onAddPackage(event: { item: Merchandise }) {
    this.saleWithTelecomService.addPackageForMerchandise(event.item, this.ticket)
  }

  onRemovePackage(event: { item: Package }) {
    this.saleWithTelecomService.removePackage(event.item, this.ticket)
  }
  // #endregion package
  ...
  onSave() {
    ...
    if (message) {
      ...
      const voucherDto = this.saleWithTelecomService.prepareVoucher();
      this.route.queryParams.subscribe((data: any) => {
  ...
  handleProcessImei(ma_imei: string) {
    ...
    const ngay_ct = new Date(this.ticket.masterInfo.ngay_ct);
    this.saleWithTelecomService.getImeiInStore(this.ma_imei, ngay_ct).subscribe(result => {
      if (result.success && result.result.length) {
