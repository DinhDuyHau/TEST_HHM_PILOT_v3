...
import { MasterInfo, Merchandise, TelecomSaleTicket, TAB_NAME } from '@app/sales-management/model/ticket/sale-with-telecom/model';
...
import { MerchandiseRequest, MasterInfoRequest } from '@app/sales-management/model/ticket/sale-with-telecom/request.model';
...
export class SaleWithTelecomService {
  discountOptions: Discount[] = [];
  isNeedCalcDiscount = true;
  ticket!: TelecomSaleTicket;
  option!: Option;
  ...
  //#region setter
  setTicket(ticket: TelecomSaleTicket, option: Option) {
    this.ticket = ticket;
    this.option = option;
  }
  ...
  initTicket(ticket: TelecomSaleTicket) {
    const userJson = localStorage.getItem('user');
    const userObj = userJson !== null && JSON.parse(userJson);

    ticket.masterInfo.ma_ct = TICKET_CODE.TELECOM;
    ...
    this.ticketApiService.getVoucherNumber(TICKET_ENTITY.TELECOM).subscribe(result => {
      ...
      if (current_soct === newSoCT) {
          ...
          this.commonService.saveVoucherNumberLocalStorage(ticket.masterInfo.so_ct, TICKET_CODE.TELECOM);
      }
    });
  ...
  resetCustomerInfo(ticket: TelecomSaleTicket) {
  ...
  // handle select no_km
  onChangePromotionalDebt(event: { item: Merchandise, index: number, checked: boolean, columnName: string }, ticket: TelecomSaleTicket) {
  ...
  calcDiscount(): Observable<ResultNoPaging<Discount>> | undefined {
    ...
    if (this.isNeedCalcDiscount) {
      return this.discountApiService.getDiscountForTicket(entity, merchandise, ma_cuahang, ma_kh, ngay_lap, service, '', TICKET_CODE.TELECOM, stt_rec);
    }
    return;
  }
  ...
  // #region service
  addServiceForMerchandise(item: Merchandise, ticket: TelecomSaleTicket) {
  ...
  // Remove service
  removeService(item: Service, ticket: TelecomSaleTicket) {
  ...
  //#region package
  addPackageForMerchandise(item: Merchandise, ticket: TelecomSaleTicket) {
  ...
  validateTicket(ticket: TelecomSaleTicket): string {
    let message = '';
