...
import { MasterInfo, Merchandise, SaleAffiliateceTicket, TAB_NAME } from '@app/sales-management/model/ticket/sale-affiliate/model';
...
import { MerchandiseRequest, MasterInfoRequest } from '@app/sales-management/model/ticket/sale-affiliate/request.model';
...
export class SaleAffiliateService {
    discountOptions: Discount[] = [];
    isNeedCalcDiscount = true;
    ticket!: SaleAffiliateceTicket;
    option!: Option;
    ...
    //#region setter
    setTicket(ticket: SaleAffiliateceTicket, option: Option) {
        this.ticket = ticket;
        this.option = option;
    }
    ...
    initTicket(ticket: SaleAffiliateceTicket) {
        const userJson = localStorage.getItem('user');
        const userObj = userJson !== null && JSON.parse(userJson);

        ticket.masterInfo.ma_ct = TICKET_CODE.AFFILIATE;
    ...
    resetCustomerInfo(ticket: SaleAffiliateceTicket) {
    ...
    // handle select no_km
    onChangePromotionalDebt(event: { item: Merchandise, index: number, checked: boolean, columnName: string }, ticket: SaleAffiliateceTicket) {
    ...
    calcDiscount(loai_ck: string = ""): Observable<ResultNoPaging<Discount>> | undefined {
        ...
        if (this.isNeedCalcDiscount) {
            return this.discountApiService.getDiscountForTicket(entity, merchandise, ma_cuahang, ma_kh, ngay_lap, service, loai_ck, TICKET_CODE.AFFILIATE, stt_rec);
        }
        return;
    }
    ...
    // #region service
    addServiceForMerchandise(item: Merchandise, ticket: SaleAffiliateceTicket) {
    ...
    // Remove service
    removeService(item: Service, ticket: SaleAffiliateceTicket) {
    ...
    //#region package
    addPackageForMerchandise(item: Merchandise, ticket: SaleAffiliateceTicket) {
    ...
    validateTicket(ticket: SaleAffiliateceTicket): string {
        let message = '';
