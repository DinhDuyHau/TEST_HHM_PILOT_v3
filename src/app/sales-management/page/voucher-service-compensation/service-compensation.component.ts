...
import { ServiceCompensationService } from './service-compensation.service';
...
@Component({
    selector: 'app-service-compensation',
    templateUrl: './service-compensation.component.html',
    styleUrls: ['./service-compensation.component.scss'],
})
export class ServiceCompensationComponent implements OnInit, AfterViewInit {
    ...
    entity = TICKET_ENTITY.SERVICE_COMPENSATION;
    action = '';
    shop = '';
    ...
    constructor(
        ...
        private serviceCompensationService: ServiceCompensationService,
        ...
        this.serviceCompensationService.setTicket(this.ticket);
    }
    ...
    ngOnInit() {
        ...
        this.route.queryParams.subscribe((data: any) => {
            if (data.key) {
                this.disableSelectStatus = false;
                this.ticketApiService.getVoucherByid(TICKET_ENTITY.SERVICE_COMPENSATION, data.key).subscribe((result) => {
                    if (result.result) {
                        ...
                        if (this.mode === MODE.UPDATE &&
                            !((result.result as any).masterInfo.status === STATUS_LIST.SERVICE_COMPENSATION.CREATE
                                || (result.result as any).masterInfo.status === STATUS_LIST.SERVICE_COMPENSATION.PENDING_PAYMENT
                                || (result.result as any).masterInfo.status === STATUS_LIST.SERVICE_COMPENSATION.PENDING_PUBLISH
                            )) {
                        ...
                        this.serviceCompensationService.loadData(result.result as any as VoucherDto);
                        this.getStatusList();
                ...
                this.disableSelectStatus = true;
                this.serviceCompensationService.initTicket(this.ticket);
                this.getStatusList();
                this.commonService.getPointRateExchange(this.ticket);
    ...
    getStatusList = () => {
        this.ticketApiService.getStatusWithOrder([{ Name: 'ma_ct', Operator: '=', Value: TICKET_CODE.SERVICE_COMPENSATION }], 'xorder,status').subscribe(result => {
    ...
    onEnterCustomerCode(ma_kh: string) {
        this.customerApiService.getOneById(ma_kh).subscribe(result => {
            if (result.success && result.result) {
                ...
                this.serviceCompensationService.resetCustomerInfo(this.ticket);
    ...
    openAddCustomerDialog(ma_kh = ''): void {
        this.commonService.openDialog(CustomerCreateDialogComponent, { ma_kh: ma_kh }, 'fullscreen-dialog')
            ...
            .subscribe((customer: Customer) => {
                customer && this.serviceCompensationService.setInfoCustomer(customer);
            });
    }
    ...
    handleAddService(service: any) {
        ...
        if (isExist) {
            ...
            this.serviceCompensationService.setInfoService(service);
        }
    }
    // click button add service
    onRemoveService(event: { item: Service }) {
        this.serviceCompensationService.removeService(event.item, this.ticket);
    }
    ...
    onChangeQuantity(event: { item: any, index: number, value: any, columnName: string }) {
        ...
        this.serviceCompensationService.calcMoney();
    }
    // #endregion service
