...
@Component({
  selector: 'search-v2-dialog',
  templateUrl: './serach-v2-dialog.component.html',
  styleUrls: ['./serach-v2-dialog.component.scss'],
})
export class SearchV2DialogComponent implements OnInit, AfterViewInit {
  dataSource: any[] = [];
  selectedData: any[] = [];
  columns!: Cell[];
  page_index = 1;
  ...
  ngOnInit(): void {
    ...
    switch (this.data.componentName) {
      ...
      case SEARCH_V2_COMPONENT_NAME.DELIVERY_EMP:
        ...
        this.defaultFilters = [filter];
        break;
      case SEARCH_V2_COMPONENT_NAME.DELIVERY_PARNER:
        this.columns = CUSTOMER_SEARCH as any;
        filter.name = 'nh_kh9';
        // filter.value = `%${this.data.keyword}%`;
        filter.value = 'NGKH21';
        ...
        this.defaultFilters = [filter];
        break;
      case SEARCH_V2_COMPONENT_NAME.BANK_ACCOUNT:
        this.columns = BANK_ACCOUNT_SEARCH as any;
        filter.name = 'tknh';
        filter.value = `%${this.data.keyword}%`;
        this.defaultFilters = [filter];
        break;
      case SEARCH_V2_COMPONENT_NAME.BANK_PUBLISH_CARD:
        this.columns = BANK_PUBLISH_CARD_SEARCH as any;
        filter.name = 'ma_nh';
        filter.value = `%${this.data.keyword}%`;
        this.defaultFilters = [filter];
        break;
      case SEARCH_V2_COMPONENT_NAME.INSTALLMENT_UNIT:
        this.columns = CUSTOMER_SEARCH as any;
        filter.name = 'ma_kh';
        filter.value = `%${this.data.keyword}%`;
        this.defaultFilters = [filter, { name: 'nh_kh9', operator: '=', value: 'NGKH88' }];
        break;
      case SEARCH_V2_COMPONENT_NAME.WALLET:
        this.columns = CUSTOMER_SEARCH as any;
        filter.name = 'ma_kh';
        filter.value = `%${this.data.keyword}%`;
        this.defaultFilters = [filter, { name: 'vdt_yn', operator: '=', value: true }];
        break;
      case SEARCH_V2_COMPONENT_NAME.SERVICE:
        this.columns = SERVICE_SEARCH as any;
        filter.name = 'ma_dv';
        filter.value = `%${this.data.keyword}%`;
        this.defaultFilters = [filter];
        break;
      case SEARCH_V2_COMPONENT_NAME.CONTRACT:
        this.columns = CONTRACT_SEARCH as any;
        break;
      case SEARCH_V2_COMPONENT_NAME.ITINERANT:
        this.columns = PROJECT_SEARCH as any;
        filter.name = 'ma_vv';
        filter.value = `%${this.data.keyword}%`;
        this.defaultFilters = [filter];
        break;
      case SEARCH_V2_COMPONENT_NAME.TYPE_MERCHANDISE:
        this.columns = TYPE_MERCHANDISE as any;
        filter.name = 'ma_vt';
        filter.value = `%${this.data.keyword}%`;
        this.defaultFilters = [filter];
        break;
      case SEARCH_V2_COMPONENT_NAME.TYPE_INVENTORY:
        this.columns = TYPE_INVENTORY as any;
        filter.name = 'ma_vt';
        filter.value = `%${this.data.keyword}%`;
        this.defaultFilters = [filter];
        break;
      case SEARCH_V2_COMPONENT_NAME.WAREHOUSE:
        this.columns = WAREHOUSE_LIST as any;
        filter.name = 'ma_vt';
        filter.value = `%${this.data.keyword}%`;
        this.defaultFilters = [filter];
        break;
      case SEARCH_V2_COMPONENT_NAME.INVOICE:
        this.columns = INVOICE_LIST as any;
        this.keyword = this.data.keyword;
        break;
      case SEARCH_V2_COMPONENT_NAME.ASM_EMPLOYEE:
        this.columns = LIST_ASM as any;
        this.keyword = this.data.keyword;
        break;
      case SEARCH_V2_COMPONENT_NAME.POS:
        ...
        break;
      case SEARCH_V2_COMPONENT_NAME.TYPE_RENEW:
        this.columns = LIST_PRICE_RENEW as any;
        break;
      case SEARCH_V2_COMPONENT_NAME.OLD_RECEIVER_SUPPLIER:
        this.columns = CUSTOMER_SEARCH as any;
        // filter.name = 'ma_kh';
        // filter.value = `%${this.data.keyword}%`;
        ...
        this.defaultFilters = [filter];
        break;
      case SEARCH_V2_COMPONENT_NAME.APPROVER_DIRECTOR:
        this.columns = LIST_BGD as any;
        this.keyword = this.data.keyword;
        break;
      case SEARCH_V2_COMPONENT_NAME.E_COMMERCIAL:
        this.columns = CUSTOMER_SEARCH as any;
        filter.name = 'nh_kh9';
        filter.value = 'NGKH77';
        filter.operator = "=";
        this.defaultFilters = [filter];
        break;
      case SEARCH_V2_COMPONENT_NAME.PACKAGE:
        ...
        this.defaultFilters = [filter];
        break;
      case SEARCH_V2_COMPONENT_NAME.STOCK_INFO:
        this.columns = STOCK_LIST as any;
        this.defaultFilters = this.data.filter || [];
        break;
      case SEARCH_V2_COMPONENT_NAME.SHOP_INFO:
        this.columns = SHOP_INFO as any;
        break;
      case SEARCH_V2_COMPONENT_NAME.SHOP_INFO:
        this.columns = SHOP_INFO as any;
        break;
      case SEARCH_V2_COMPONENT_NAME.EMPLOYEE:
  ...
  findDataSourceLocal(filters: any[], page_index: number, page_size: number) {
    let res = this.selectedData.filter((e: any) => {
      let isMatch = true;
  ...
  onSelectItem(event: { item: any }): void {
    const existingItem = this.selectedData.find((e: any) => e.ma_kho === event.item.ma_kho);
    if (existingItem) {
      existingItem.selected = true;
    }
    const items = this.selectedData.filter((e: any) => e.selected);
  ...
  onSave() {
    const item = this.selectedData.filter((e: any) => e.selected);
...
export const SEARCH_V2_COMPONENT_NAME = {
  MERCHANDISE: 0,
  CUSTOMER: 1,
  IMEI: 2,
  DELIVERY_EMP: 3,
